...
function generarDetallePDF(idDeFicha){
    $.get('https://patrimoniocultural.edomex.gob.mx/obtenerFichasDetalladas.php?fichas=["' +  idDeFicha + '"]', function( data ) {
        ...
        addIntro(doc, ficha.archivos, ficha.museo, ficha.id, ficha.usuario, ficha.status);
        var seccionProcedencia = obtenerSeccionPorNombre(ficha.secciones, "Procedencia");
        var seccionMedidasMaximas = obtenerSeccionPorNombre(ficha.secciones, "Dimensiones");
        var seccionMedidasBM = obtenerSeccionPorNombre(ficha.secciones, "Medidas máximas Base\/Marco");
        printLeftContent(doc, ficha.archivos, seccionProcedencia, seccionMedidasMaximas, seccionMedidasBM);
...
function addHeader(doc, coleccion){
    ...
    doc.setFontSize(13).text(120, 11, 'Colección').setFontSize(16).text(120, 18, coleccion);
}
...
function addIntro(doc, archivos, museo, id, usuario, status){
    if(archivos != null && archivos.length > 0){
        var imageData;
        if(archivos[0].endsWith("jpg") || archivos[0].endsWith("jpeg") || archivos[0].endsWith("png"))
            requestFile("https://patrimoniocultural.edomex.gob.mx/" + archivos[0], insertarImagen, doc);
        else if(archivos[0].endsWith("pdf"))
            requestFile("https://patrimoniocultural.edomex.gob.mx/images/thumbs_PDF.jpg", insertarImagen, doc);
        else if(archivos[0].endsWith("mp4") || archivos[0].endsWith("avi") || archivos[0].endsWith("3gp"))
            requestFile("https://patrimoniocultural.edomex.gob.mx/images/video_icon.png", insertarImagen, doc);

        //doc.addImage(imageData, 'JPEG', 15, 40, 180, 160);
        
    }
    doc.setFontSize(9).setFontStyle("bold").setTextColor("#000000").text(65, 32, "Museo").setFontStyle("normal").setTextColor("#777777").text(65, 37, museo);
    doc.setFontSize(9).setFontStyle("bold").setTextColor("#000000").text(65, 44, "Ubicación del Museo").setFontStyle("normal").setTextColor("#777777").text(65, 49, "Pendiente");
    doc.setFontSize(9).setFontStyle("bold").setTextColor("#000000").text(65, 56, "ID del registro").setFontStyle("normal").setTextColor("#777777").text(65, 61, id);
    doc.setFillColor("#B1D939").rect(145, 33, 20, 6, 'F');
    doc.setFontSize(9).setFontStyle("bold").setTextColor("#000000").text(145, 32, "Status").setFontStyle("bold").setTextColor("white").text(147, 37, status);
    doc.setFontSize(9).setFontStyle("bold").setTextColor("#000000").text(145, 44, "Fecha de creación").setFontStyle("normal").setTextColor("#777777").text(145, 49, idToFecha(id));
    doc.setFontSize(9).setFontStyle("bold").setTextColor("#000000").text(145, 56, "Usuario de registro").setFontStyle("normal").setTextColor("#777777").text(145, 61, usuario);
    doc.setDrawColor("#E7E7E8").line(6, 66, 209, 66, "S");
}

function insertarImagen(imageData, doc){
    doc.addImage(imageData, 'JPEG', 20, 30, 20, 20);
}
...
function populateMultimediaSection(imageData, doc, index){
    var xLevel = 8;
    var yLevel = 83;
    doc.addImage(imageData, 'JPEG', xLevel + (xLevel * index), yLevel, 10, 9);
}

function printLeftContent(doc, archivos, procedencia, medidas, medidasBase){
    var startX = 6;
    var startY = 75;
    doc.setFontSize(12).setFontStyle("bold").setTextColor("#000000").text(startX, startY, "Galería");
    doc.setDrawColor("#E7E7E8").rect(6, 80, 60, 15, "S");
    
    if(archivos != null)
        for(var i = 0 ; i < archivos.length ; i++){
            requestFile("https://patrimoniocultural.edomex.gob.mx/" + archivos[i], populateMultimediaSection, doc, i);
        }
    ...
    startX = 6;
    startY = 105;
    doc.setFontSize(11).setFontStyle("bold").setTextColor("#000000").text(startX, startY, "Procedencia");
    startY += 5;
    var contentHeight = 0;
    if(procedencia != null){
        ...
        if(campos.length == 0){
            ...
            for(var i = 0 ; i < campos.length ; i++){
                contentHeight += 4;
                doc.setFontSize(9).setFontStyle("bold").setTextColor("#000000").text(10, startY + contentHeight, campos[i].nombre)
                contentHeight += 4;
                var currentVal = (campos[i].valorReal != null) ? campos[i].valorReal : campos[i].valor;
                doc.setFontStyle("normal").setTextColor("#777777").text(10, startY + contentHeight, currentVal);
                contentHeight += 1;
            }
    } else {
        contentHeight = 10;
    ...
    doc.setFontSize(11).setFontStyle("bold").setTextColor("#000000").text(startX, startY, "Medidas");
    startY += 5;
    var contentHeight = 0;
    if(medidas != null){
        ...
        if(campos.length == 0){
            ...
            for(var i = 0 ; i < campos.length ; i++){
                ...
                doc.setFontStyle("normal").setTextColor("#777777").text(10, startY + contentHeight, campos[i].valor);
                contentHeight += 1;
            }
    } else {
        contentHeight = 10;
    ...
    startY += contentHeight + 10;
    doc.setFontSize(11).setFontStyle("bold").setTextColor("#000000").text(startX, startY, "Medidas Base/Marco");
    startY += 5;
    var contentHeight = 0;
    if(medidasBase != null){
...
function printRightContent(doc, secciones){
    ...
    for(var i = 0 ; i < secciones.length ; i++){
        ...
        doc.setFontSize(12).setFontStyle("bold").setTextColor("#000000").text(startX, startY, seccion.nombreSeccion);
        var campos = seccion.campos;
        for(var j = 0 ; j < campos.length ; j++){
            var campo = campos[j];
            startY += 8;
            doc.setFontSize(9).setFontStyle("normal").setTextColor("#000000").text(startX, startY , campo.nombre);
            if(campo.tipo == "catalogo")
                    campo.valor = campo.valorReal;
            startY += 4;
            doc.setFontStyle("normal").setTextColor("#777777").text(startX, startY, campo.valor);
            
            if(campos[j + 1] != null){
                //agregar un segundo a lado
                startY -= 12;
                startX += 75;
                var campo = campos[j + 1];
                startY += 8;
                doc.setFontSize(9).setFontStyle("normal").setTextColor("#000000").text(startX, startY , campo.nombre);
                if(campo.tipo == "catalogo")
                        campo.valor = campo.valorReal;
